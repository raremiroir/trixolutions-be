<script lang="ts">
   ...
   import { 
         ...
         List, ListItem, Button, Link } from "$comp/core";
   import { Hero, Card, TwicPic, Modal } from "$comp/common";
   import { 
         Accordeon, AccordeonItem, RatingCard, FeaturesSlider, 
         FeaturesSlide, FeaturesModalContent } from "$comp/content";
...
<header>
   <Hero titleSmall imgSrc="home/working-genius.webp" imgAlt="No alt">
      <span slot="title">{titleCase($LL.open_sessions.level_2.title())}</span>   
   </Hero>
</header>
...
<Main noMargin cta>
   ...
   <SectionWrapper name="hybride-lencioni-leertraject">
      ...
      <P large>
         {#each Object.entries($LL.open_sessions.level_2.intro) as [key, item]}
            {@html item()}
         {/each}
      </P>
      <List>
      ...
      <Accordeon bind:active>
         {#each Object.entries($LL.open_sessions.level_2.accordeon) as [key, item]}
            <AccordeonItem 
               id={Number(key)+1} 
               ...
               {#each Object.entries(item.content) as [contentKey, content]}
                  ...
                  <Title type="h5">{firstLetterCase($LL.open_sessions.workshop())}</Title>
                  <P large>
   ...
   <SectionWrapper name="hybride-lencioni-leertraject-praktische-info">
      <div class="flex flex-row w-full gap-16 items-center justify-start" slot="title">
         ...
         <div class="">
            <Title type="subheader" smaller italic>
               {titleCase($LL.open_sessions.level_2.advanced_coaching())}
            </Title>
            <Title type="h3" smaller thin>
               {titleCase($LL.open_sessions.level_2.title())} - {titleCase($LL.open_sessions.level_2.level2())}
            </Title>
         </div>
      </div>
      ...
      <div class="grid grid-cols-2 gap-4 xl:gap-8">
         ...
         <Modal icon="mdi:account-group-outline">
            ...
            <P large>
               {$LL.open_sessions.level_2.target_audience()}
               <br/>
            </P>
            <Link href="/{$locale}/open-sessies/hybride-lencioni-leertraject">
               {$LL.open_sessions.level_2.target_audience_extra()}
            </Link>
         </Modal>
         ...
         <Modal icon="mdi:tools">
            ...
            <P large>
               {$LL.open_sessions.level_2.approach()}
            </P>
         </Modal>
      </div>
         ...
         {#each pageData as session}
            {#if session.type === 'level_2'}
               <PricingCard
                  introWidth="h-fit" 
                  price={session.price}>
                  <span slot="subtitle">{$LL.open_sessions.level_2.advanced_coaching()}</span>
                  <span slot="title">
                     {$LL.open_sessions.level_2.title()} <br class="md:hidden"/>
                     <span class="hidden md:inline">&#45; </span>{$LL.open_sessions.level_2.level2()}
                  </span>
            
                  <p slot="intro">
                     {#each Object.entries($LL.open_sessions.level_2.features_intro) as [key, item]}
                        {@html item()}
                     {/each}
                  </p>
                  
                  <span slot="sub">{$LL.open_sessions.vat_not_included()}</span>
                  <List slot="features" class="w-4/5">
                     {#each Object.entries($LL.open_sessions.level_2.features) as [key, feature]}
                        <ListItem icon="check">{feature()}</ListItem>
                     {/each}
                  </List>
            
                  <span slot="btn">
                     <span class="font-light">{$LL.open_sessions.subscribe_to()}</span><br class="sm:hidden"/>
                     <span>{$LL.open_sessions.level_2.title()}</span>
                  </span>
